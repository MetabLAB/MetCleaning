#' Calculate fold change for MetFlowData.
#'
#' @title FoldChange
...
FoldChange <- function(MetFlowData = MetFlowData,
  ...
  if (length(unique(group)) != 2) {stop("No two class data!!!")}
  group2.index <- which(group == to[1])
  group1.index <- which(group == to[2])

  if (ratio == "median") {
  foldchange <- apply(subject, 1, function(x) {median(x[group2.index]+1)/median(x[group1.index]+1)})
  }
  if (ratio == "mean") {
  foldchange <- apply(subject, 1, function(x) {mean(x[group2.index]+1)/mean(x[group1.index]+1)})
  }

  if (any(colnames(tags) == "foldchange")) {
  tags[,"foldchange"] <- foldchange
  }
  else {tags <- cbind(tags, foldchange)}

  MetFlowData[["tags"]] <- tags
  MetFlowData[["foldchange"]] <- paste(to[1]," vs ",to[2],"(",ratio,")", sep = "")
  return(MetFlowData)
}
