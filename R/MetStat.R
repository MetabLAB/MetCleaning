...
MetStat <- function(MetFlowData = MetFlowData,
    ...
    if (new.group == T) {
      ...
      save(met.data.new.group, file = file.path(path.inter, "met.data.new.group"))
    }
    ...
    if (pca.analysis) {
      ...
      cat("PCA analysis\n")
      ##PCA analysis
    PCAanalysis(MetFlowData = met.data,
                QC = QC,
                log.scale = log.scale,
    ...
    if (pls.analysis) {
      ...
      cat("PLS analysis\n")
    ##PLS analysis
    PLSanalysis(MetFlowData = met.data,
                log.scale = log.scale,
    ...
    cat("---------------------------------------------------------------------\n")
    cat("Univariate test\n")
    met.data <- UnivariateTest(MetFlowData = met.data,
                               test.method = test.method,
                               adjust.method = adjust.method,
                               log.scale = log.scale)
    #save data
    met.data.uni.test <- met.data
    save(met.data.uni.test, file = file.path(path.inter, "met.data.uni.test"))
    }

    ##MarkerSelection
    cat("---------------------------------------------------------------------\n")
    cat("Marker selection\n")
    met.data <- MarkerSelection(MetFlowData = met.data,
                                foldchange = foldchange,
                                p = p,
                                vip = vip,
    ...
    save(met.data.after.stat, file = file.path(path.inter, "met.data.after.stat"))

    ##OutputMetFlowData
    OutputMetFlowData(MetFlowData = met.data,
    ...
    VolcanoPlot(MetFlowData = met.data,
                x = x,
                y = y,
                z = z,
                col = c("black", "firebrick1"),
    ...
    cat("---------------------------------------------------------------------\n")
    cat("Marker show\n")
    MarkerShow(MetFlowData = met.data,
               beeswarm = beeswarm,
               path = file.path(path, "marker selection"))
    options(warn = 0)
