...
#' @export
#' @seealso \code{\link{t.test}}, \code{\link{p.adjust}} and \code{\link{wilcox.test}}
...
# browser()
  ...
  if (length(class) != 2 & test.method == "t" | test.method == "wilcox") {
  ...
  if(any(colnames(tags) == "p")) {tags[,"p"] <- p}
  else {tags <- cbind(tags, p)}

  if(any(colnames(tags) == "p.correct")) {tags[,"p.correct"] <- p.correct}
  else {tags <- cbind(tags,p.correct)}
  if (test.method == "anova") {
    tags <- cbind(tags, tuk.p)
  }

  if(any(colnames(tags) == "feature.auc")) {tags[,"feature.auc"] <- feature.auc}
  else {tags <- cbind(tags,feature.auc)}
